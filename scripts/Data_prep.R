...
library(readstata13)
path <- "Census.dta"
Census <- read.dta13(path)
## if .sav (SPSS)
if (!require("foreign")) install.packages("foreign")
library(foreign)
path <- "Census.sav"
Census <- read.spss(path, to.data.frame=TRUE)
...
Census$Adn2_name <- recode(Census$Adn2_name, incorrectname1 = "Correct_Name_1")
Census$Adn2_name <- recode(Census$Adn2_name, incorrectname2 = "Correct_Name_2")
Census$Adn2_name <- recode(Census$Adn2_name, incorrectname3 = "Correct_Name_3")
Census$Adn2_name <- recode(Census$Adn2_name, incorrectname4 = "Correct_Joined_Name_4")
Census$Adn2_name <- recode(Census$Adn2_name, incorrectname5 = "Correct_Joined_Name_4")
...
Census$hh_size <- recode(Census$hh_size, 'pob 1'=1, 'pob 2'=2, 'pob 3'=3, 'pob 4'=4, 'pob 5'=5, 
...
Census$n_rooms <- recode(Census$n_rooms, '1 habitaci<f3>n'=1, '2 habitaciones'=2, 
...
write.csv(Census, file = "Census_clean.csv", row.names=FALSE)
