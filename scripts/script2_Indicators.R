...
clean_path <- "Data\\interrim_data_files\\Census_clean.csv"
if (OS == "unix"){    
  census_path <- str_replace_all(census_path, '\\\\', '/\\')    # file path adjustment for Macs
}
Census_c <- read.csv("Census_clean.csv")

attach(Census_c)    # keep census in R's immediate memory cache for ease of coding
...
final_path <- "Data\\interrim_data_files\\Census_indicators_full.csv"
if (OS == "unix"){    
  final_path <- str_replace_all(final_path, '\\\\', '/\\')    # file path adjustment for Macs
}
write.csv(Census_c, file = final_path, row.names=FALSE)
...
write.csv(Census_c, file = indicators_path, row.names=FALSE) 
...
setdiff(indicators_table$Adm2, adm2@data$NAME_2)
library("dplyr")
indicators_table$Adm2_name <- dplyr::recode(indicators_table$Adm2_name, incorrectname1 = "Correct_Name_1", 
                                            incorrectname2 = "Correct_Name_2", 
                                            incorrectname3 = "Correct_Name_3", 
                                            incorrectname4 = "Correct_Name_4", 
                                            incorrectname5 = "Correct_Name_5")
colnames(adm2@data)[colnames(adm2@data)=="NAME_2"] <- "Adm2"      # join by column names should match too
...
# save as shapefile!
shp_path <- "Data\\interrim_data_files\\Map_indicators.shp"
if (OS == "unix"){    
  shp_path <- str_replace_all(shp_path, '\\\\', '/\\')    # file path adjustment for Macs
}
writeOGR(Country_map, shp_path, layer="Country_map", driver="ESRI Shapefile")
...
path <- "Data\\imgs\\Total_def.pdf"
if (OS == "unix"){    
  path <- str_replace_all(path, '\\\\', '/\\')    # file path adjustment for Macs
}
save_tmap(td, path)    # save map as PDF
...
path <- "Data\\imgs\\Def_quali.pdf"
if (OS == "unix"){    
  path <- str_replace_all(path, '\\\\', '/\\')    # file path adjustment for Macs
}
save_tmap(qd, path)    # save map as PDF
...
path <- "Data\\imgs\\Def_quanti.pdf"
if (OS == "unix"){    
  path <- str_replace_all(path, '\\\\', '/\\')    # file path adjustment for Macs
}
save_tmap(qnd, path)    # save map as PDF
...
tm_shape(Country_map2) + 
  tm_polygons("def_light_acc", title = "Proportion hh", breaks=seq(0, 1, by=.1), border.col = "white") + 
...
tm_shape(Country_map2) + 
  tm_polygons("def_water_acc", title = "Proportion hh", breaks=seq(0, 1, by=.1), border.col = "white") + 
...
tm_shape(Country_map2) + 
  tm_polygons("def_sewer_acc", title = "Proportion hh", breaks=seq(0, 1, by=.1), border.col = "white") + 
...
tm_shape(Country_map2) + 
  tm_polygons("cohabitation", title = "Proportion hh", breaks=seq(0, 1, by=.1), border.col = "white") + 
...
tm_shape(Country_map2) + 
  tm_polygons("overcrowding", title = "Proportion hh", breaks=seq(0, 1, by=.1), border.col = "white") + 
...
tm_shape(Country_map2) + 
  tm_polygons("acute_overcrowd", title = "Proportion hh", breaks=seq(0, 1, by=.1), border.col = "white") + 
...
print('')
print('finished running script 2: Indicators')
print('')
